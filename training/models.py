...
class VideoEncoder(nn.Module):
    def __init__(self):
        ...
        num_fts = self.backbone.fc.in_features               #to get the number of features in the output layer of pretrained part
        self.backbone.fc = nn.Sequential(
            nn.Linear(num_fts, 128),                           # map number of features to 128
            nn.ReLU(),
            nn.Dropout(0.2)
        )
...
#NOW ALL THE ENCODERS RETURN A TENSOR OF SIZE [BATCH_SIZE, 128] FOR EACH MODALITY
#NOW THESE WILL BE PASSED TO A FUSION LAYER WHICH WILL CONCATENATE THEM AND THEN PASS THROUGH A CLASSIFICATION HEAD FOR EMOTION AND SENTIMENT PREDICTION
#THE FUSION LAYER WILL BE A LINEAR LAYER FOLLOWED BY BATCH NORMALIZATION, RELU ACTIVATION AND DROPOUT
#THE CLASSIFICATION HEADS WILL BE TWO SEPARATE LINEAR LAYERS FOR EMOTION AND SENTIMENT PREDICTION  
...
class MultimodalSentimentModel(nn.Module):
    ...
    def forward(self, text_inputs, video_frames, audio_features):
        ...
        combined_features = torch.cat([
            ...
            video_features,
            audio_features
        ], dim=1)  #  # this will return something like this [batch_size, 128 * 3]
